    ...
    @else
        ...
        <div class="space-y-4">
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 dark:bg-white/5">
                            <tr>
                                ...
                                <th class="px-3 py-2 text-start font-medium">Ism</th>
                                <th class="px-3 py-2 text-start font-medium">Login</th>
                                <th class="px-3 py-2 text-start font-medium">Parol</th>
                        ...
                        <tbody>
                            @foreach ($rows as $row)
                                <tr wire:key="import-row-{{ $row['row'] }}" class="border-t border-gray-100 dark:border-white/5">
                                    ...
                                    <td class="px-3 py-2 font-mono">{{ $row['username'] }}</td>
                                    <td class="px-3 py-2 font-mono">{{ $row['password'] }}</td>
                                    <td class="px-3 py-2">
                                        @php
            ...
            <div class="flex gap-2">
                ...
                >
                    {{ $counts['new'] }} ta foydalanuvchini import qilish
                </x-filament::button>
            </div>
        </div>
    @endif
